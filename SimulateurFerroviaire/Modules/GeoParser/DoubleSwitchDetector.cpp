...
void DoubleSwitchDetector::detectAndAbsorb()
{
    ...
    for (auto& cluster : clusters)
    {
        ...
        {
            auto* sw = switchIndex.count(cluster[i]) ? switchIndex.at(cluster[i]) : nullptr;
            if (!sw) continue;
            if (i + 1 < cluster.size()) sw->nextSwitchId = cluster[i + 1];
            if (i > 0)                  sw->prevSwitchId = cluster[i - 1];
        }
        ...
        for (std::size_t i = 0; i + 1 < cluster.size(); ++i)
        {
            ...
            if (!swA || !swB) continue;
            std::string linkId;
            for (const auto& bid : swA->branchIds)
            {
                if (!straightIndex.count(bid)) continue;
                const auto& nb = straightIndex.at(bid)->neighbourIds;
                if (std::find(nb.begin(), nb.end(), cluster[i + 1]) != nb.end()) { linkId = bid; break; }
            }
            if (!linkId.empty())
                absorbLinkSegment(cluster[i], cluster[i + 1], linkId, switchIndex, straightIndex, segmentsToRemove);
        }
    ...
    {
        m_straights.erase(std::remove_if(m_straights.begin(), m_straights.end(),
            [&](const StraightBlock& s) { return segmentsToRemove.count(s.id) > 0; }),
            m_straights.end());
        LOG_INFO(m_logger, std::to_string(segmentsToRemove.size()) + " segment(s) absorbé(s)");
    }
}
    ...
    const std::unordered_map<std::string, StraightBlock*>& straightIndex)
{
    ...
    for (const auto& [id, sw] : switchIndex) adjacency[id] = {};

    for (const auto& [segId, st] : straightIndex)
    {
        if (st->lengthMeters > m_doubleLinkMaxMeters) continue;
        std::vector<std::string> owners;
        for (const auto& [swId, sw] : switchIndex)
        {
            if (!sw->isOriented() || sw->rootBranchId == segId) continue;
            for (const auto& bid : sw->branchIds)
    ...
    for (const auto& [startId, neighbours] : adjacency)
    {
        ...
        while (!queue.empty())
        {
            ...
            visited.insert(cur); cluster.push_back(cur);
...
double DoubleSwitchDetector::estimateAccessibleChainLength(
    const std::string& branchId, int junctionNodeId,
    const std::unordered_map<std::string, StraightBlock*>& straightIndex) const
{
    ...
    double total = it->second->lengthMeters;

    std::set<int> junctionNodes;
    for (const auto& [id, nodeId] : m_switchIdToNodeId) junctionNodes.insert(nodeId);
    ...
    while (!stack.empty())
    {
        ...
        for (const auto& [nb, eid] : it2->second)
        {
            ...
            if (eit != m_topologyGraph.edges.end()) total += eit->second.lengthMeters;
            stack.push_back(nb);
        }
    }
    return total;
...
void DoubleSwitchDetector::absorbLinkSegment(
    const std::string& switchIdA, const std::string& switchIdB,
    ...
    std::set<std::string>& segmentsToRemove)
{
    ...
    if (!link->coordinates.empty())
    {
        LatLon mid = link->coordinates[link->coordinates.size() / 2];
        swA->doubleLinkMidCoordinate = mid;
    ...
    {
        auto& nb = st.neighbourIds;
        auto pos = std::find(nb.begin(), nb.end(), linkSegmentId);
        if (pos == nb.end()) continue;
        nb.erase(pos);
        for (const auto& pid : { switchIdA, switchIdB })
            if (std::find(nb.begin(), nb.end(), pid) == nb.end()) nb.push_back(pid);
        std::sort(nb.begin(), nb.end());
    }
    LOG_DEBUG(m_logger, "Absorbé : " + linkSegmentId + " (" + switchIdA + " ↔ " + switchIdB + ")");
}
...
void DoubleSwitchDetector::validateSwitches()
{
    ...
    for (auto& s : m_straights) stIdx[s.id] = &s;
    std::set<std::string> swIds;
    for (const auto& sw : m_switches) swIds.insert(sw.id);

    int warnings = 0;
    for (const auto& sw : m_switches)
    {
        if (!sw.isOriented()) continue;
        const std::string& nId = sw.normalBranchId.value_or("");
        const std::string& dId = sw.deviationBranchId.value_or("");

        if (sw.isDoubleSwitch)
        {
            int swCount = (int)swIds.count(nId) + (int)swIds.count(dId);
            if (swCount != 1)
            {
                LOG_WARNING(m_logger, sw.id + " : double aiguille avec " + std::to_string(swCount) + " branche(s) Switch (attendu : 1)");
                ++warnings;
            }
        }
        else if (!stIdx.count(nId) || !stIdx.count(dId))
        {
            LOG_WARNING(m_logger, sw.id + " : type(s) de branche invalide(s) — normal=" + nId + ", deviation=" + dId);
            ++warnings; continue;
        }
    ...
    else LOG_WARNING(m_logger, "Validation CDC : " + std::to_string(warnings) + " violation(s)");
}
